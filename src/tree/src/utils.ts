  ...
  traverse(
    ...
    (node) => {
      ...
      if (filter(pattern, node)) {
        highlightKeySet.add(get(node as any, keyField))
        for (let i = path.length - 2; i >= 0; --i) {
          if (!keys.has(get(path[i] as any, keyField))) {
            keys.add(get(path[i] as any, keyField))
          } else {
            return
          }
        }
...
export function filterTree (
  ...
  function visit (t: TreeOption[]): void {
    t.forEach((n) => {
      path.push(n)
      if (filter(pattern, n)) {
        visitedTailKeys.add(get(n as any, keyField) as Key)
        highlightKeySet.add(get(n as any, keyField) as Key)
        for (let i = path.length - 2; i >= 0; --i) {
          const key: Key = get(path[i] as any, keyField)
      ...
      const children = get(n, childrenField) as TreeOption[] | undefined
      if (children) {
        visit(children)
      }
      path.pop()
  ...
  function build (t: TreeOption[], sibs: TreeOption[]): void {
    t.forEach((n) => {
      const key = get(n as any, keyField) as Key
      ...
      if (children) {
        if (isVisitedTail) {
          ...
          build(children, get(clonedNode, childrenField) as TreeOption[])
        }
      } else {
        sibs.push(n)
      }
