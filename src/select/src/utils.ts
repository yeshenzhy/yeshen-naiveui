...
export function createTmOptions (
  ...
  > = {
    ...
    getKey (option: SelectMixedOption): string | number {
      ...
      return get((option as SelectBaseOption | SelectIgnoredOption), valueField) as NonNullable<SelectOption['value']>
    },
    getChildren (option) {
      return get(option, childrenField) as SelectBaseOption[]
    }
  }
  return options
}
...
export function filterOptions (
  ...
  function traverse (options: SelectMixedOption[]): SelectMixedOption[] {
    ...
    for (const option of options) {
      if (getIsGroup(option)) {
        const children = traverse(get(option, childrenField) as SelectBaseOption[])
        if (children.length) {
          filteredOptions.push(
            set(option, childrenField, children)
...
export function createValOptMap (
  ...
  options.forEach((option) => {
    if (getIsGroup(option)) {
      ;(get(option, childrenField) as SelectBaseOption[]).forEach(
        (selectGroupOption: SelectBaseOption) => {
          valOptMap.set(
            get(selectGroupOption, valueField) as NonNullable<
      ...
      valOptMap.set(
        get(option, valueField) as NonNullable<SelectBaseOption['value']>,
