  ...
  setup(props) {
      ...
      if (remote) {
        ...
        if (multiple) {
          ...
          selectedOptionsRef.value?.forEach((option) => {
            memoValOptMap.set(
              get(option, valueField) as NonNullable<SelectOption['value']>,
              option
            )
          })
        } else {
          ...
          if (option) {
            memoValOptMap.set(
              get(option, props.valueField) as NonNullable<
                SelectOption['value']
              >,
              option
            )
    ...
    function getCreatedOptionIndex(optionValue: string | number): number {
      ...
      return createdOptions.findIndex(
        (createdOption) =>
          (get(createdOption, props.valueField) as NonNullable<
            SelectOption['value']
          >) === optionValue
      )
    }
    function handlePatternInput(e: InputEvent): void {
      ...
      // const { value } = ((e.target as unknown as any)?.shadowRoot && e.composed
      //   ? { value: e.data }
      //   : e.target) as unknown as HTMLInputElement
      const { value } = e['savedTarget']

      patternRef.value = value
      const { tag, remote } = props
      doSearch(patternRef.value)
      if (tag && !remote) {
        if (!value) {
          beingCreatedOptionsRef.value = emptyArray
          return
        ...
        ///TODO:可能有问题，设置的labelField/valueField这块
        ...
        if (
          compitableOptionsRef.value.some(
            (option) =>
              get(option, valueField) === get(optionBeingCreated, valueField)
          ) ||
